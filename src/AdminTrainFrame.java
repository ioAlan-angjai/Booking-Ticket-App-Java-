...
public class AdminTrainFrame extends JFrame {
    ...
    private static final String ID_COL = "TRAIN_ID";
    private static final String SEAT_COL = "SEAT_AVAIL"; 

    public AdminTrainFrame() {
        setTitle("Admin - Manage Trains");
        setSize(1080, 700);
    ...
    private void loadItems(String searchKeyword) {
        model.setRowCount(0);
        String sql = "SELECT " + ID_COL + ", OPERATOR, ORIGIN, DESTINATION, PRICE, " + SEAT_COL + ", IMAGE_PATH FROM " + TABLE_NAME;
    ...
    private void createItem() {
        try {
            ...
            String sql = "INSERT INTO " + TABLE_NAME + " (" + ID_COL + ", OPERATOR, ORIGIN, DESTINATION, PRICE, " + SEAT_COL + ", IMAGE_PATH) VALUES (?, ?, ?, ?, ?, ?, ?)";
            ...
            JOptionPane.showMessageDialog(this, "Isi field ID, Price, dan Seat dengan benar.", "Input Error", JOptionPane.WARNING_MESSAGE);
        } catch (SQLException ex) {
    ...
    private void updateItem() {
        try {
            ...
            String sql = "UPDATE " + TABLE_NAME + " SET OPERATOR=?, ORIGIN=?, DESTINATION=?, PRICE=?, " + SEAT_COL + "=?, IMAGE_PATH=? WHERE " + ID_COL + "=?";
            try (Connection conn = Koneksi.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setString(1, op);
                ps.setString(2, or);
                ps.setString(3, dest);
